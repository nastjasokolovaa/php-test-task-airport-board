...
class AirportRepository extends AbstractRepository
{
    ...
    private const TIME_ZONE_DATA_KEY = 'timeZone';
    ...
    {
        $this->assertKeysExists($airportData, [self::NAME_DATA_KEY, self::CITY_DATA_KEY, self::TIME_ZONE_DATA_KEY]);

        return new Airport(
            $airportData[self::CODE_DATA_KEY],
            $airportData[self::NAME_DATA_KEY],
            $airportData[self::CITY_DATA_KEY],
            $airportData[self::TIME_ZONE_DATA_KEY],
        );
    }
